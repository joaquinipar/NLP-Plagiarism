...
best_score = 0
best_score_title = ""
general_plagiarism = 0
number_general_plagiarism = 0
...
def plagiarism(doc1,doc2=None):
    ...
    if doc2 is None: # Checks the web
        ...
        print(f"Analizing {doc1} file")


        global best_score
        global best_score_title
        global general_plagiarism
        global number_general_plagiarism
        global plagiarism
        global j
        for j in search(title(doc1), tld="com.ar",lang='es', num=10, stop=5, pause=2): 
            
            #print("Found!")
            #print(f"Link: {j}")

            print(f"Analizing {title1} and {title(j)}")
        ...
        stats = general_plagiarism / number_general_plagiarism
        print("----------------------")
        if(need_scraping == True):
            print(f"The best plagiarism score comparison between paragraphs is '{best_score_title}' with a score of {best_score}")
            print(f"Percentage of general plagiarism in the text {stats}")
        else:
            print(f"The best plagiarism score comparison between paragraphs is '{best_score}")
            print(f"Percentage of general plagiarism in the text {stats}")
...
def detect_plagiarism(string1,string2):
    ...
    for sent2 in doc2.sents:
        ...
        for sent1 in doc1.sents:
            ...
            general_plagiarism += sim
            number_general_plagiarism += 1
